...
% Allocate and intialize the post-survival value functions, the entry
% probabilities, and the probability of sure survival.
...
% \begin{equation}
% \begin{split}
% v_S(n,c) &=  \rho \mathbb E\bigg[\; & \overbrace{ \pi(n,C')}^{\textbf{flowSurplus}}  - \overbrace{\left[1 - G_W\left(\omega ^ 2 - \log v_S(n,C')\right)\right]}^{\textbf{partialExp}} \\
...
thetaW2 = Param.omega ^ 2;
gridTrans = exp(Settings.logGrid)';

for n = Settings.nCheck:-1:1

    % % initialize the iteration counter and the criterion value
    iter = 0;
    vSdiff = 1;

    % % take the nth row out of vS, transpose it, and iterate on this column vector
    vSn = vS(n, :)';
    ...
    valueAdditionalEntry = ...
        sum(pEntrySet((n + 1):end, :) .* vS((n + 1):end, :))';
    ...
    while (vSdiff > Settings.tolInner && iter < Settings.maxIter)
        ...
        % % only compute the log once
        logvSn = log(vSn);

        % % compute the probability that incumbents survive with prob 1
        pStayn = normcdf(logvSn, -0.5 * thetaW2, Param.omega);

        % % compute partial expectation of cost shock
        partialExp = 1 - normcdf(0.5 * thetaW2 - logvSn, 0, Param.omega);

        % % assemble value function update
    ...
    vS(n, :) = vSn;
    pStay(n, :) = pStayn;
    pEntry(n, :) = normcdf(logvSn - log((1 + Param.phi)), -0.5 * thetaW2, Param.omega);
