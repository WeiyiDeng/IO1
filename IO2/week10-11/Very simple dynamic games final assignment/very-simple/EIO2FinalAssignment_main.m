...
nfirms = csvread('nfirms.csv',1,0);
population = csvread('population.csv',1,0);
cbsa = csvread('cbsa.csv',1,0);

population = population./10000;                 % rescale population
...
Settings.nCheck = max(nfirms);             % nCheck = 9
% Settings.nCheck = 10;
...
Settings.cCheck = 200;
% Settings.cCheck = 500;
Settings.lowBndC = min(population)/1.25;
Settings.uppBndC = max(population)*1.25;
...
C_grid_point = zeros(length(population),1);
grid_points = exp(Settings.logGrid);
for i= 1:length(population)
    index = sum(population(i)>grid_points);
    if abs(grid_points(index)-population(i))<abs(grid_points(index+1)-population(i))
        C_grid_point(i) = index;
    else
        C_grid_point(i) = index+1;
    end
end
...
Settings.rCheck = length(unique(cbsa));    % 573
Settings.tCheck = length(cbsa)/Settings.rCheck;        % 10
Settings.tBurn = 100;
...
% Param.k = [1.8, 1.4, 1.2, 1, 0.9, 1.1, 1.3, 1.6, 0.7];
Param.k = ones(1,Settings.nCheck);
Param.phi = 10;
Param.omega = 1;
Param.demand.muC = 0;
...
Data.C = reshape(C_grid_point,Settings.tCheck,Settings.rCheck);
Data.N = reshape(nfirms,Settings.tCheck,Settings.rCheck);
...
Settings.estimates2phi = @(x) x(Settings.nCheck+1);
Settings.estimates2omega = @(x) x(Settings.nCheck+2);
...
% Estimates.step2 = [Param.k (1*nCheck), Param.phi, Param.omega]
...
display(Estimates.step3)
display(Estimates.se)

format bank
coefficients = Estimates.step3';
coefficients(12:13) = coefficients(12:13).*100;
se = Estimates.se';
var_names = {'k(1)','k(2)','k(3)','k(4)','k(5)','k(6)','k(7)','k(8)','k(9)',...
    'phi','omega','demand.muC*10^2','demand.sigmaC*10^2'};
resultTable = table(coefficients, se, 'RowNames', var_names)
writetable(resultTable,'resultTable.xls','Sheet',1,'Range','A1','WriteRowNames',true)
format short
