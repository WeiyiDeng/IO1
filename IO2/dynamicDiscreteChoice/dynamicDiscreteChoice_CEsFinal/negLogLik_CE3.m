...
function [nll,negScore,informationMatrix] = ...
         negLogLik_CE3(choices,iX,supportX,capPi,beta,delta,rho,flowpayoffs,bellman,fixedPoint,tolFixedPoint,...
         capU0Vector,capU1Vector)
%{
...
if isempty(capU0Vector) && isempty(capU1Vector)
    [capU0,capU1] = fixedPoint(u0,u1,capPi,rho,tolFixedPoint,bellman,[],[]);
else
    capU0 = reshape(capU0Vector,nSuppX,2);
    capU1 = reshape(capU1Vector,nSuppX,2);
end
deltaU = capU1-capU0;
...
% 	dPsi_dTheta = [[zeros(2*nSuppX,1);ones(2*nSuppX,1)] [zeros(2*nSuppX,1);supportX;supportX] [zeros(2*nSuppX,1);-ones(nSuppX,1);zeros(nSuppX,1)]];
    dPsi_dTheta = [[zeros(2*nSuppX,1);ones(2*nSuppX,1)] [zeros(2*nSuppX,1);supportX;supportX] [zeros(2*nSuppX,1);-ones(nSuppX,1);zeros(nSuppX,1)] repmat([zeros(2*nSuppX,1);-ones(nSuppX,1);zeros(nSuppX,1)],1,20)];      % w: how to change?
    %{
