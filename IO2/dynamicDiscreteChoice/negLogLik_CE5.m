...
%}
function nll = ...
         negLogLik_CE5(choices,iX,supportX,capPi,beta,delta,rho,drawsX,drawsEpsilon1,drawsEpsilon0,DeltaU,...
...
%}
nPeriods = size(choices,1);
nFirms = size(choices,2);
nSimuFirms = size(drawsEpsilon1,2);
nSimuPeriods = size(drawsEpsilon1,1);

% generate choices and collect flow utilities
...
%     for At_prev = 0                             % At-1
        ...
        A_hat = zeros(nSimuPeriods,nSimuFirms);
        u_hat = zeros(nSimuPeriods,nSimuFirms);
        A_hat(1,:) = A_hat_t;
        u_hat(1,:) = u_hat_At;
        for t = 2:nSimuPeriods
            ...
            for i = 1:length(A_hat_t)
                A_hat_tau(i) = DeltaU(X_hat_tau(i),A_hat_t(i)+1) + Epsilon1_hat_tau(i) - Epsilon0_hat_tau(i)>=0;
            end
            A_hat(t,:) = A_hat_tau;
            for i = 1:length(A_hat_t)
...
% compute discounted future utilities capital U
CapU_cube_hat = zeros(nSuppX,2,2);
Epsilon_cube = drawsEpsilon0;
Epsilon_cube(:,:,2) = drawsEpsilon1;
for Xt = 1:nSuppX
    for At_prev = [0 1]
        ...
        for t = 2:nSimuPeriods
            for i = 1:nSimuFirms
                Epsilon_Atau(t,i) = Epsilon_cube(t,i,A_hat11(t,i)+1);
            end
            CapU11 = CapU11+rho^(t-1)*(u_hat11(t,:)+Epsilon_Atau(t,:));
        end
%         CapU11_bar = mean(CapU11);
        CapU11_bar_At0 = mean(CapU11(A_hat11(1,:)==0));     % for each starting point, average out over # of simulated times
...
% compute log pseudo-likelihood as objective function
choice_prev = choices(1,:);
ll_mat = zeros(size(choices));
for t = 2:nPeriods
    iX_t = iX(t,:);
    choice_t = choices(t,:);
    for i = 1:nFirms
        ll_mat(t,i) = exp(CapU_cube_hat(iX_t(i),choice_prev(i)+1,choice_t(i)+1))/...
            (exp(CapU_cube_hat(iX_t(i),choice_prev(i)+1,1))+exp(CapU_cube_hat(iX_t(i),choice_prev(i)+1,2)));
    end
    choice_prev = choice_t;
end
ll_mat(1,:) = [];
nll = -sum(sum(log(ll_mat)));        % w: remember to log !!!
% w: how to compute weighted distance objective function (what is the weighting matrix?) ??   
...
% % p = choices + (1-2*choices).*pExit(iX+nSuppX*laggedChoices);         % pExit(iX+nSuppX*laggedChoices) is T*N matrix   100*1000
% % % w: same as following:
