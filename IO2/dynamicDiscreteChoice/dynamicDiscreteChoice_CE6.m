...
\paragraph{Software Requirements}
    ...
    Running the code documented in this file requires a recent version of
\url{http://www.mathworks.com/products/matlab/}{\textsc{Matlab}} with its
    \url{http://www.mathworks.nl/help/optim/index.html}{\textsc{Optimization Toolbox}}. The code can easily be adapted to use the \url{http://www.ziena.com/knitro.html}{\textsc{Knitro}} optimization package instead of or with \textsc{Matlab}'s \textsc{Optimization Toolbox} (see Section \ref{script}).\footnote{The code has been tested with \textsc{Matlab} 2014a and 2014b, with its \textsc{Optimization Toolbox} and the \textsc{Knitro} 9.0 optimization package, on an iMac with OS X 10.9. A basic student version of \textsc{Knitro} is available from \url{http://www.ziena.com/knitro.html}{Ziena Optimization} for free.}
...
%}
[choices,iX] = simulateData_CE6(deltaU_type1,deltaU_type2,theta1,capPi,nPeriods,nFirms);
%{
...
%}
objectiveFunction = @(parameters)negLogLik_CE6(choices,iX,supportX,capPi,parameters(1:2),...
    [delta_type1(1);parameters(3);parameters(4)],parameters(5),rho,@flowpayoffs,@bellman,@fixedPoint,tolFixedPoint)
% objectiveFunction = @(parameters)negLogLik_CE6(choices,iX,supportX,capPi,parameters(1:2),...
%     [delta_type1(1);parameters(3);parameters(4)],theta1,rho,@flowpayoffs,@bellman,@fixedPoint,tolFixedPoint)
%{
...
%}
startvalues = [-1;-0.1;0.5;2;0.5];
% startvalues = [-1;-0.1;0.5;2];
%{
...
[maxLikEstimates,~,exitflag,~,~,~,hessian] = fmincon(objectiveFunction,startvalues,[],[],[],[],lowerBounds,upperBounds,[],OptimizerOptions)
%{
...
disp([[beta;delta_type1(2);delta_type2(2);theta1] startvalues maxLikEstimates standardErrors]);
disp('w: The code seems not able to identify the parameters. Every time it iterates, it tries out very large coefficient values and produces Inf objective function value')
%{
