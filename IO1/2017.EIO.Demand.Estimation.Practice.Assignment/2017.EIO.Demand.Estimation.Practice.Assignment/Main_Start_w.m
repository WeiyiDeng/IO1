        ...
        for l = 1:total_products_market_m
            ...
            if j==l                                    % own price elasticity
                elasticities_mat(j,l) = price_coef*price(j_ID)/(1-sigma_coef)*...
                    (1-sigma_coef*withinGroup_share(j_ID)-(1-sigma_coef)*share(j_ID));
            elseif group(j_ID)==group(l_ID)            % cross price elasticities same group
                ...
                elasticities_mat(j,l) = -price_coef*price(l_ID)/(1-sigma_coef)*...
                    (sigma_coef*withinGroup_share(l_ID)+(1-sigma_coef)*share(l_ID));
            end
        end
    end
    eta_matrices{m} = elasticities_mat;
...
disp(['mean own price elasticities across markets: ' num2str(elast1)])
disp(['mean cross price elasticities across markets: ' num2str(elast2)])
disp('computed price elasticities seems incorrect')

% w: computed price elasticities seems incorrect, why ??

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% BLP ESTIMATION OF RANDOM COEFFICIENTS LOGIT MODEL %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% w: use exp(y) (y is DV of logit) as starting guess of delta
% dim 970*1000
...
x0 = [3.1278, 3.0239, 0.5310, 0.5134, -2.0092, 0.8398, 0.4907, 0.4710, 0.4786]';
opts    = optimset('Display','iter','TolCon',1E-6,'TolFun',1E-10,'TolX',1E-10, 'MaxIter',3);
...
% end
% 
% xi_jm_hat = sigma_jm - [ones(Total,1) XX]*theta1;               % Total*1     970*1
% 
% g = IV'*xi_jm_hat;                                              % nIV*1       13*1
% 
% % phi = g*g';
% 
% % new_W = phi\eye(length(g));                              % gives error?
% 
